...
namespace SupplyApp.ViewModel
{
    ...
    internal class InitiatorDirViewModel
    {
        private readonly InitiatorDirModel _initiatorDirModel = new();

        public ObservableCollection<Initiator> Data { get; set; }
        ...
        public Initiator SelectedItem { get; set; } = null!;

        public InitiatorDirViewModel()
        {
            ...
            Data = new ObservableCollectionListSource<Initiator>(_initiatorDirModel.GetAll());
        ...
        private void CollectionChanged(object? sender, NotifyCollectionChangedEventArgs e)
        {
            switch (e.Action)
            {
                case NotifyCollectionChangedAction.Add:
                    if (e.NewItems != null)
                        foreach (Initiator exec in e.NewItems)
                            SnackBarMessageQueue.Enqueue(_initiatorDirModel.Add(exec)
                                ? "Запись добавлена"
                                : "Такая запись уже существует");
                    break;
                case NotifyCollectionChangedAction.Replace:
                    if (e.OldItems != null)
                        foreach (Initiator exec in e.OldItems)
                            SnackBarMessageQueue.Enqueue(_initiatorDirModel.Update(exec)
                                ? "Запись изменена"
                                : "Такая запись уже существует");
                    break;
                case NotifyCollectionChangedAction.Remove:
                    if (e.OldItems != null)
                        foreach (Initiator exec in e.OldItems)
                            SnackBarMessageQueue.Enqueue(_initiatorDirModel.Remove(exec)
                                ? "Запись удалена"
        ...
        public ICommand AddCommand => new RelayCommand((obj) =>
        {
            ...
            Data.Add(new Initiator() { Name = EditBox });
